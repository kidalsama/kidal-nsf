...
export default class Application {
  ...
  public static async runTest(
    serviceName: string,
    applicationConfigType: string = "test"
  ): Promise<Application> {
    // 伪造命令行参数
  ...
  // 开始启动流程
  private static async startBootstrapSequence(
    argv: string[],
    testing: boolean
  ): Promise<Application> {
    // timer
    ...
    Application.LOG.info(
      `Started ${env.applicationConfig.id} in ${sec} seconds`
    );
  ...
  private async boot(env: Environment) {
    ...
    if (fs.existsSync(entrySrc)) {
      const main = require(PathUtils.replaceExt(entrySrc, ".js")).default;
